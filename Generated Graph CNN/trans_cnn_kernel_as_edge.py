...
class Node_OP(nn.Module):
  def __init__(self, Node, inplanes, inplanes_array, wh, batch_size, device):
    ...
    for i in range(self.input_nums):
      ...
      self.set_zero[str(self.id) + '_' + str(i)] = 0
  ...
  def forward(self, *input):
    if self.init_input_nums > 1:
      k = 0
      out = torch.zeros(self.batch_size, self.init_input_nums, self.wh, self.wh).to(self.device)
      if self.deleted[list(self.deleted.keys())[0]] == 0:
        #out[:,0,:,:] = self.sigmoid(self.mean_weight[list(self.mean_weight.keys())[0]]) * self.convs[list(self.convs.keys())[0]](input[0]).squeeze(1)
        out[:,0,:,:] = torch.zeros(input[0].shape[0], input[0].shape[2], input[0].shape[3]).to(self.device)
        k += 1
      else:
        if self.set_zero[list(self.convs.keys())[0]] == 0:
          out[:,0,:,:] = torch.zeros(input[0].shape[0], input[0].shape[2], input[0].shape[3]).to(self.device)
          self.set_zero[list(self.convs.keys())[0]] == 1

      
      for i in range(1, self.init_input_nums):
        if self.deleted[list(self.deleted.keys())[i]] == 0:
          #out[:,i,:,:] = self.sigmoid(self.mean_weight[list(self.mean_weight.keys())[k]]) * self.convs[list(self.convs.keys())[k]](input[i]).squeeze(1)
          out[:,0,:,:] = torch.zeros(input[0].shape[0], input[0].shape[2], input[0].shape[3]).to(self.device)
          k += 1
        else:
          if self.set_zero[list(self.convs.keys())[0]] == 0:
            out[:,i,:,:] = torch.zeros(input[0].shape[0], input[0].shape[2], input[0].shape[3]).to(self.device)
            self.set_zero[list(self.convs.keys())[i]] == 1
...
class StageBlock(nn.Module):
  def __init__(self, graph, inplanes, wh, batch_size, device):
    ...
    for node in self.nodes:
      ...
      self.nodeop.append(Node_OP(node, inplanes, inplanes_array, wh, batch_size, self.device))
    '''
...
class Trans_CNN_kernel_edge(nn.Module):
  def __init__(self, cfg):
    ...
    self.conv3 = StageBlock(graph, c, 16, self.batch_size, self.device)

    graph = build_graph(cfg['nodes'], cfg)
    self.graph_arr.append(graph)
    save_graph(graph, './graph/graph_{}_conv4.yaml'.format(str(cfg["type"])))
    self.conv4 = StageBlock(graph, self.nodes_num - 1, 8, self.batch_size, self.device)
...
ax.set_axis_off()
plt.show()
x = torch.randn(1,3,32,32).to(torch.device('cuda:0'))
print(net(x).shape)
...
print(t2 - t1)
t1 = time.time()
t0 = torch.randn(32,1,32,32).cuda()
for i in range(1000):
  t = torch.cat([t, t0], dim = 1)
t2 = time.time()
print(t2 - t1)
#%%
t0 = torch.randn(32,1000,32,32).cuda()
t1 = time.time()
for i in range(1000):
  t0[:,1,:,:]=torch.randn(32,32,32)
t2 = time.time()
print(t2 - t1)
# %%
'''